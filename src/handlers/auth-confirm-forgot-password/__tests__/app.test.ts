...
describe('auth-confirm-forgot-password handler', () => {
  ...
  it('returns 400 when newPassword is too short (< 6 chars)', async () => {
    ...
    expect(res.statusCode).toBe(400);
    expect(JSON.parse(res.body).message).toContain('6-18');
  });

  it('returns 400 when newPassword is too long (> 18 chars)', async () => {
    const res = await handler(makeEvent({ email: 'test@example.com', code: '123456', newPassword: 'A'.repeat(19) }), {} as any, () => {}) as any;
  ...
  it('returns 429 when rate limited', async () => {
    ddbMock.on(GetCommand).resolves({ Item: { key: 'forgot-confirm:test@example.com', count: 10, ttl: Math.floor(Date.now() / 1000) + 900 } });
