...
describe('update-stats stream handler', () => {
  ...
  // -- Recomputes monthly hashes after daily record writes -------------------

  it('recomputes monthly hashes after daily record writes', async () => {
    const trade = makeTrade({ openDate: '2026-04-15T09:30:00Z' });

    // GSI query for queryTradesForDay returns the trade key
    ...
    // Monthly hash recomputation queries DailyStats table for the month
    // and returns the daily record with date and tradeHash
    ddbMock.on(QueryCommand, { TableName: 'test-daily-stats' }).resolves({
      Items: [{ date: '2026-04-15', tradeHash: 'abc123def456' + '0'.repeat(52) }],
    });

    const event = makeStreamEvent([
      makeStreamRecord('INSERT', trade, undefined, 'evt-monthly-hash'),
    ]);

    await handler(event, {} as any, () => {});

    // recomputeMonthlyHashes queries daily records for the month
    const queryCalls = ddbMock.commandCalls(QueryCommand);
    const monthlyQuery = queryCalls.find(
      (c) =>
        c.args[0].input.ExpressionAttributeValues?.[':skStart'] === 'acc-1#2026-04-01' &&
        c.args[0].input.ExpressionAttributeValues?.[':skEnd'] === 'acc-1#2026-04-31',
    );
    expect(monthlyQuery).toBeDefined();

    // Verify PutCommand was called for monthly hash record
    const putCalls = ddbMock.commandCalls(PutCommand);
    const monthlyHashPut = putCalls.find(
      (c) => c.args[0].input.Item?.sk === 'acc-1#MONTH#2026-04',
    );
    expect(monthlyHashPut).toBeDefined();
    expect(monthlyHashPut!.args[0].input.Item!.userId).toBe('user-1');
    expect(monthlyHashPut!.args[0].input.Item!.accountId).toBe('acc-1');
    expect(monthlyHashPut!.args[0].input.Item!.month).toBe('2026-04');
    expect(monthlyHashPut!.args[0].input.Item!.monthHash).toHaveLength(64);
  });

  it('does not fail stream processing when monthly hash recomputation fails', async () => {
    const trade = makeTrade({ openDate: '2026-04-15T09:30:00Z' });

    // GSI query for queryTradesForDay succeeds
    ddbMock.on(QueryCommand, { IndexName: 'trades-by-date-gsi' }).resolves({
      Items: [{ userId: 'user-1', tradeId: 'trade-1' }],
    });
    ddbMock.on(BatchGetCommand).resolves({
    ...
    // Monthly hash query (BETWEEN on DailyStats) fails
    let queryCallCount = 0;
    ddbMock.on(QueryCommand, { TableName: 'test-daily-stats' })
      .callsFake(() => {
        queryCallCount++;
        throw new Error('Monthly hash query failed');
      });

    const event = makeStreamEvent([
      makeStreamRecord('INSERT', trade, undefined, 'evt-monthly-fail'),
    ]);

    const result = (await handler(event, {} as any, () => {})) as any;

    // Should NOT fail — monthly hash errors are caught and logged
    expect(result.batchItemFailures).toHaveLength(0);
  });
