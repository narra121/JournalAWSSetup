...
export const handler: APIGatewayProxyHandlerV2 = async (event) => {
  ...
  try {
    ...
    if (!preferences.notifications) preferences.notifications = {};
    if (data.tradeReminders !== undefined) preferences.notifications.tradeReminders = data.tradeReminders;
    if (data.weeklyReport !== undefined) preferences.notifications.weeklyReport = data.weeklyReport;
    if (data.goalAlerts !== undefined) preferences.notifications.goalAlerts = data.goalAlerts;
    ...
    return {
      ...
      body: JSON.stringify(envelope({ statusCode: 200, data: { notifications: preferences.notifications } }))
    };
  } catch (error: any) {
