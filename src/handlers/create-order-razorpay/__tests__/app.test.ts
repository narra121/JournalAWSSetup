...
describe('create-order-razorpay handler', () => {
  ...
  it('returns 500 with correct message when Razorpay times out', async () => {
  ...
  it('returns 400 when body is invalid JSON', async () => {
    ...
    // JSON.parse will throw, caught by outer catch => 500
    expect(res.statusCode).toBe(500);
    const body = JSON.parse(res.body);
    expect(body.errorCode).toBe('INTERNAL_ERROR');
  });
  ...
  it('returns 401 when token is malformed (no sub claim)', async () => {
    ...
    const event = makeEvent({ amount: 99 }, { headers: { authorization: `Bearer ${badHeader}.${badPayload}.sig` } });
    const res = await handler(event, {} as any, () => {}) as any;
  ...
  it('uses custom currency when provided', async () => {
    ...
    const res = await handler(makeEvent({ amount: 10, currency: 'USD' }), {} as any, () => {}) as any;
  ...
  it('defaults to INR currency when not specified', async () => {
    await handler(makeEvent({ amount: 50 }), {} as any, () => {}) as any;
  ...
  it('passes custom notes along with userId to Razorpay', async () => {
    await handler(
      makeEvent({ amount: 99, notes: { plan: 'premium', coupon: 'SAVE10' } }),
      {} as any,
      () => {},
    ) as any;
  ...
  it('converts amount to paise (multiplies by 100)', async () => {
    ...
    const res = await handler(makeEvent({ amount: 299 }), {} as any, () => {}) as any;
