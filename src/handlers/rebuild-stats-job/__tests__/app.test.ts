...
describe('rebuild-stats-job handler', () => {
  ...
  // -- Monthly hash recomputation --------------------------------------------

  it('recomputes monthly hashes after rebuilding daily records', async () => {
    stubRecentUsers(['user-1']);
    stubUserTrades([
      makeTrade({
        tradeId: 't1',
      ...
      makeTrade({
        tradeId: 't2',
        openDate: '2026-04-15T10:00:00Z',
        closeDate: '2026-04-15T14:00:00Z',
      }),
    ]);

    // For monthly hash recomputation, queries on DailyStats table for the month
    // return daily records with date + tradeHash
    ddbMock.on(QueryCommand, { TableName: 'test-daily-stats' })
      .callsFake((input: any) => {
        const skStart = input.ExpressionAttributeValues?.[':skStart'];
        if (skStart && skStart.includes('#2026-04-01')) {
          // Monthly hash query — return daily records for the month
          return {
            Items: [
    ...
    // Verify a monthly hash record was written
    const putCalls = ddbMock.commandCalls(PutCommand);
  ...
  it('does not delete monthly hash records during orphan cleanup', async () => {
    stubRecentUsers(['user-1']);
    stubUserTrades([
      makeTrade({ tradeId: 't1', openDate: '2026-04-10T10:00:00Z' }),
    ]);

    // Monthly hash query returns daily record
    ddbMock.on(QueryCommand, { TableName: 'test-daily-stats' })
      .callsFake((input: any) => {
        const skStart = input.ExpressionAttributeValues?.[':skStart'];
        if (skStart && skStart.includes('#2026-04-01')) {
          return {
            Items: [{ date: '2026-04-10', tradeHash: 'hash-a' }],
          };
        }
        // Orphan cleanup: return a monthly hash record and the daily record
        return {
          Items: [
            { userId: 'user-1', sk: 'acc-1#2026-04-10' },
            { userId: 'user-1', sk: 'acc-1#MONTH#2026-04' },
          ],
        };
      });
    ...
    // Verify that DeleteCommand was NOT called for the monthly hash record
    const deleteCalls = ddbMock.commandCalls(DeleteCommand);
    const monthlyDelete = deleteCalls.find(
      (c) => c.args[0].input.Key?.sk === 'acc-1#MONTH#2026-04',
    );
    expect(monthlyDelete).toBeUndefined();
  });
